...
# This script applies the DDI signal detection algorithm for rhabdomyolysis using drug pairs being co-reported with rhabdomyolysis in at least 5 reports in FAERS.
...
# Read data file with FAERS counts for rhabdomyolysis
counts <- read.csv("rhabdomyolysis_faers_counts.csv")
# Generate df with all counts
rhabdomyolysis_cases <- data.frame(dde_tuple = counts$dde_tuple,
                                    N_111 = counts$n_111,
...
for (k in 1:nrow(rhabdomyolysis_cases)) {
  d <- as.list(rhabdomyolysis_cases[k,])
...
rhabdomyolysis_post_prob <- cbind(rhabdomyolysis_cases, cases_log_lik, 
...
rhabdomyolysis_post_prob$logsumexp_H_..1 <- rowLogSumExps(
  matrix(unlist(rhabdomyolysis_post_prob[, c(11, 13, 15, 17)]), ncol=4))

# Negative outcome
# LogSumExp of loglik_H_000, loglik_H_100, loglik_H_010, loglik_H_110
rhabdomyolysis_post_prob$logsumexp_H_..0 <- rowLogSumExps(
  matrix(unlist(rhabdomyolysis_post_prob[,c(10, 12, 14, 16)]), ncol=4))

# Difference of LogSumExp values
rhabdomyolysis_post_prob$ratio <- rhabdomyolysis_post_prob$logsumexp_H_..1 -
  rhabdomyolysis_post_prob$logsumexp_H_..0

# Add posterior probabilities
rhabdomyolysis_post_prob$log_post_H_..1 <- log(exp(rhabdomyolysis_post_prob$log_post_H_001) + 
                                                  exp(rhabdomyolysis_post_prob$log_post_H_101) +
                                                  exp(rhabdomyolysis_post_prob$log_post_H_011) + 
                                                  exp(rhabdomyolysis_post_prob$log_post_H_111))
rhabdomyolysis_post_prob$log_post_H_..0 <- log(exp(rhabdomyolysis_post_prob$log_post_H_000) + 
                                                  exp(rhabdomyolysis_post_prob$log_post_H_100) +
                                                  exp(rhabdomyolysis_post_prob$log_post_H_010) + 
                                                  exp(rhabdomyolysis_post_prob$log_post_H_110))

rhabdomyolysis_post_prob$sum_posterior_pos <- log(exp(rhabdomyolysis_post_prob$log_post_pos_H_001) + 
                                                     exp(rhabdomyolysis_post_prob$log_post_pos_H_101) + 
                                                     exp(rhabdomyolysis_post_prob$log_post_pos_H_011) + 
                                                     exp(rhabdomyolysis_post_prob$log_post_pos_H_111))

rhabdomyolysis_post_prob$post_odds_ratio <- rhabdomyolysis_post_prob$sum_posterior_pos - 
  rhabdomyolysis_post_prob$log_post_H_..0

# Write output to a .csv file
write.csv(
  rhabdomyolysis_post_prob,
  "output/rhabdomyolysis_sda_scores.csv"
)
